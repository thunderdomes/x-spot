...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSMutableArray *dataPass=[[NSMutableArray alloc]initWithCapacity:1];
   tempfvaluepas = c.textLabel.text;
    
    if (btnSender.tag ==1 || btnSender.tag ==2)
    {
        if ([tempfvaluepas isEqualToString:@"5 Transaksi Terakhir"])
        {
            //btnSender.titleLabel.text = @"5 Transaksi Terakhir";
            UIImage *buttonImage = [UIImage imageNamed:@"pam-ipad-button-historitransaksi-01-184x29.png"];
            [btnSender setImage:buttonImage forState:UIControlStateNormal];
            fvaluepass = @"5";
			[dataPass addObject:@"1"];
			[dataPass addObject:@"5"];
			[[NSNotificationCenter defaultCenter] postNotificationName:@"TransaksiVal" object:dataPass];
			[dataPass removeAllObjects];
            
        } else
            if ([tempfvaluepas isEqualToString:@"10 Transaksi Terakhir"]) {
                UIImage *buttonImage = [UIImage imageNamed:@"pam-ipad-button-historitransaksi-02-184x29.png"];
                [btnSender setImage:buttonImage forState:UIControlStateNormal];
                fvaluepass = @"10";
				[dataPass addObject:@"1"];
				[dataPass addObject:@"10"];
				[[NSNotificationCenter defaultCenter] postNotificationName:@"TransaksiVal" object:dataPass];
				[dataPass removeAllObjects];
                
            } else
                if ([tempfvaluepas isEqualToString:@"15 Transaksi Terakhir"]) {
                    UIImage *buttonImage = [UIImage imageNamed:@"pam-ipad-button-historitransaksi-03-184x29.png"];
                    [btnSender setImage:buttonImage forState:UIControlStateNormal];
                    fvaluepass = @"15";
					[dataPass addObject:@"1"];
					[dataPass addObject:@"15"];
					[[NSNotificationCenter defaultCenter] postNotificationName:@"TransaksiVal" object:dataPass];
					[dataPass removeAllObjects];
                } else
                    if ([tempfvaluepas isEqualToString:@"6 bulan Terakhir"]) {
                        UIImage *buttonImage = [UIImage imageNamed:@"pam-ipad-button-historitransaksi-04-184x29.png"];
                        [btnSender setImage:buttonImage forState:UIControlStateNormal];
                        fvaluepass = @"1";
						[dataPass addObject:@"2"];
						[dataPass addObject:@"1"];
						[[NSNotificationCenter defaultCenter] postNotificationName:@"TransaksiVal" object:dataPass];
						[dataPass removeAllObjects];
                    } else
                        if ([tempfvaluepas isEqualToString:@"1 Tahun Terakhir"])
                        {
                            UIImage *buttonImage = [UIImage imageNamed:@"pam-ipad-button-historitransaksi-05-184x29.png"];
                            [btnSender setImage:buttonImage forState:UIControlStateNormal];
                            fvaluepass = @"2";
							[dataPass addObject:@"2"];
							[dataPass addObject:@"2"];
                            ...
                            if ([tempfvaluepas isEqualToString:@"3 Tahun Terakhir"])
                            {
                                UIImage *buttonImage = [UIImage imageNamed:@"pam-ipad-button-historitransaksi-06-184x29.png"];
                                [btnSender setImage:buttonImage forState:UIControlStateNormal];
                                
                                fvaluepass = @"3";
								[dataPass addObject:@"2"];
								[dataPass addObject:@"3"];
								[[NSNotificationCenter defaultCenter] postNotificationName:@"TransaksiVal" object:dataPass];
								[dataPass removeAllObjects];
                                ...
                                {
                                    UIImage *buttonImage = [UIImage imageNamed:@"pam-ipad-button-historitransaksi-07-184x29.png"];
                                    [btnSender setImage:buttonImage forState:UIControlStateNormal];
    ...
    if (btnSender.tag ==55)
    {
    if ([tempfvaluepas isEqualToString: @"Panin Dana Maksima"])
    {
        custodianID = @"21648"; fvaluepass = @"1";
        
    }
    if ([tempfvaluepas isEqualToString: @"Panin Dana Prima"])
    {
        custodianID = @"1027816"; fvaluepass = @"19";
    }
    if ([tempfvaluepas isEqualToString: @"Panin Dana Syariah Saham"])
    {
        custodianID = @"21648";fvaluepass = @"47";
    }
    if ([tempfvaluepas isEqualToString: @"Panin Dana Bersama Plus"])
    {
        custodianID = @"1027818";fvaluepass = @"44";
    }
    //if ([tempfvaluepas isEqualToString: @"Panin Dana Bersama"]) custodianID = @"27";
    if ([tempfvaluepas isEqualToString: @"Panin Dana Unggulan"] || [tempfvaluepas isEqualToString:@"Reksadana Panin Dana Unggulan"])
    {
        custodianID = @"1027815"; fvaluepass = @"10";
    }
    if ([tempfvaluepas isEqualToString: @"Panin Dana Syariah Berimbang"])
    {
        
        custodianID = @"21648";fvaluepass = @"48";
    }
    //if ([tempfvaluepas isEqualToString: @"Panin Dana USD"]) custodianID = @"18";
    if ([tempfvaluepas isEqualToString: @"Panin Dana Prioritas"]){
        custodianID = @"21648";    fvaluepass = @"49";
    }
    if ([tempfvaluepas isEqualToString: @"Panin Dana Utama Plus 2"])
    {
        custodianID = @"21648";fvaluepass = @"15";
    }
    if ([tempfvaluepas isEqualToString: @"Panin Gebyar Indonesia II"] || [tempfvaluepas isEqualToString: @"Reksadana Panin Gebyar Indonesia II"])
    {
        custodianID = @"1027817";fvaluepass = @"23";
    }
    if ([tempfvaluepas isEqualToString: @"Panin Dana Likuid"])
    {
        custodianID = @"1027814";fvaluepass = @"46";
    }
...
//    //NSUInteger lastIndex = templastIndex+1;
    ...
    //{
        ...
        appDelegate.namaRek = [[NSString alloc]initWithString:tempfvaluepas];
        ...
        NSLog(@"app delegate fvalue:%@",appDelegate.fvalueGlobalString);
   ...
   // CekTransaction *cek = [[CekTransaction alloc]init];
    //[cek cekTransaksi:self.btnSender];
